...
class Register {
  ...
  private $uri    = 'http://sikita.freeddns.org/emrapi/api/registrasi_pasien';
  ...
  $signature = hash_hmac('sha256', $consID."&".$tStamp, $secretKey, true); // Pembuatan Tanda Tangan Untuk 
 
  $encodedSignature = base64_encode($signature); // Hasil Tanda Tangan Untuk  Setelah di Encode

  $url = "http://192.168.100.138/source/emrapi/api/registrasi_pasien"; // Alamat API
...
public function createRegistrasi($data_pasien,$data_registrasi,$data_perawatan,$data_waktu_tunggu,$dataInacbg)
  {   
        

        $datastring = array("data_pasien" => $data_pasien,
                             "data_registrasi" => $data_registrasi, 
                             "data_perawatan" => $data_perawatan,
                             "data_inacbg" => $dataInacbg,
                             "data_waktu_tunggu" => $data_waktu_tunggu); 

        // print_r($datastring); die;

        $completeurl = $this->uri;
    $response = $this->callAPI('POST', $completeurl, $datastring, 'application/json; charset=utf-8');
    return $response;
  }
