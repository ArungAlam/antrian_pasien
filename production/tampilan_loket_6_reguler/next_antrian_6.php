<?php
    ...
    function GetAntri($status) 
    {
          ...
          $sql = " select b.cust_usr_nama , a.reg_antri_suara, a.reg_antri_nomer, a.reg_antri_id  ,a.id_poli
                  ...
                  where a.reg_antri_suara = '0' and a.id_loket='6' 
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
        ...
        $tbContent[$i][$counter][TABLE_ISI] = '<a href="next_antrian_6.php?id_antri='.$dataTable[$i]["reg_antri_id"].'"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/edit.png" alt="Proses" title="Proses" border="0" ></a>';
        ...
        if($dataTable[$i]["id_poli"]==6) $jenisNama="POLI DOTS";
   ...
   function GetMasuk($status) 
   {
     ...
     $depKonfig = $dtaccess->Fetch($sql); 
      ...
      $sql = " select b.cust_usr_nama , a.reg_antri_suara, a.reg_antri_nomer ,reg_antri_id  ,a.id_loket , a.id_poli,
              ...
              where a.reg_antri_suara = 'A' and a.id_loket='6'
              and a.id_dep = ".QuoteValue(DPE_CHAR,$depId)." and a.reg_antri_tanggal = ".QuoteValue(DPE_DATE,$tgl)."
              order by reg_antri_nomer asc"; 
      $dataTable = $dtaccess->FetchAll($sql);
     ...
     for($i=0,$counter=0,$n=count($dataTable);$i<$n;$i++,$counter=0){
        ...
        $tbContent[$i][$counter][TABLE_ISI] = '<a href="next_antrian_6.php?id_antri='.$dataTable[$i]["reg_antri_nomer"].'"><b>'.$dataTable[$i]["reg_antri_nomer"].'</b></a>';
			  $tbContent[$i][$counter][TABLE_ALIGN] = "center";
        $counter++;
        ...
        if($dataTable[$i]["id_loket"]==6) $loket="LOKET 6";
        
        $tbContent[$i][$counter][TABLE_ISI] = "&nbsp;".$loket;
			  $tbContent[$i][$counter][TABLE_ALIGN] = "left";
        $counter++;
        
        if(!$dataTable[$i]["id_reg"] && $dataTable[$i]["id_loket"]=="6"){
        $tbContent[$i][$counter][TABLE_ISI] = '<a target = "_blank" href="'.$ROOT.'data_pasien/registrasi_pasien_awal.php?id_klinik_waktu_tunggu='.$dataTable[$i]["klinik_waktu_tunggu_id"].'&id_loket='.$dataTable[$i]["id_loket"].'&reguler=1"><img hspace="2" width="32" height="32" src="'.$ROOT.'gambar/icon/cari.png" alt="Proses" title="Proses" border="0" ></a>';
			  } else {
    ...
    // cek data nomer antriannya dahulu //
     ...
     // buat flag A jika gak ada antrian //
         $sql = "update klinik.klinik_antrian_reguler set antri_suara='A' where antri_id = ".QuoteValue(DPE_NUMERIC,$dataAntrian[0]["antri_id"])." and id_dep = ".QuoteValue(DPE_CHAR,$depId);    		 
...
<body class="nav-sm" onLoad="setTimeout('delayer()',1000)">
  <div class="container body">
    <div class="main_container">
        <?php require_once($LAY."sidebar_loket.php") ?>
              ...
              <div class="title_left">
                <h1>NOMOR ANTRIAN : <? echo "<b>".$dataPasienAntriReg["reg_antri_nomer"]."</b>&nbsp;-&nbsp;".$poliNama;?></h1>
                   <ul class="nav navbar-right panel_toolbox">
                      <button id="btnPanggil" name="btnPanggil" type="submit" value="PANGGIL LOKET 6" class="btn btn-primary" onClick="ProsesAntrian('<? echo $_GET["id_antri"]?>','6')" border="0" />Panggil Loket 6</button>                      <li class="dropdown">
                    </ul>
              </div>
            </div>
             <?php } ?>
