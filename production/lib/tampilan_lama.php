...
//		if ($in_month<10) $in_month = "0".$in_month;
     ...
     * @access public
     */
	function RenderBody($in_css,$isCal=false,$in_title=null,$in_event=null,$isGrid=false,$in_cssPrint=null)
	{
        global $APLICATION_ROOT,$ROOT;
        $strBody = '<html>'."\n";
        $strBody.= '<head>'."\n";
        $strBody.= '<meta http-equiv="Content-Type" content="text/html; charset=utf-8">'."\n";        
        $strBody.= '<meta name="description" content="Program Klinik & Dokter Praktek GRATIS berbasis Online Realtime, untuk Klinik anda,sangat mudah pengunaannya, simple dan praktis, sudah termasuk SMS Gateway, IPAD dan Antrian Pasien,fasilitas lengkap garansi lifetime dengan harga terjangkau." />'."\n"; 
        ...
        $strBody.= '<link href="'.$APLICATION_ROOT.'gambar/icon_sikita.gif" rel="Shortcut Icon">'."\n";
        $strBody.= '<link href="'.$APLICATION_ROOT.'lib/css/'.$in_css.'" media="screen"  rel="stylesheet" type="text/css">'."\n";

          if($in_cssPrint){
             $strBody.= '<link href="'.$APLICATION_ROOT.'lib/css/'.$in_cssPrint.'" media="print"  rel="stylesheet" type="text/css">'."\n";
          }
