<?php
     ...
     $lokasi = $ROOT."gambar/foto_pasien";
          ...
          $sql = "select * from klinik.klinik_antrian_reguler where id_dep = ".QuoteValue(DPE_CHAR,$depId)." and id_poli = ".QuoteValue(DPE_CHAR,$poliId)." and id_loket = ".QuoteValue(DPE_CHAR,$loketId)." 
                  order by antri_id asc";
          //echo $sql;
          //die();
          $DataAntrian = $dtaccess->Fetch($sql);
            // cek jika ada pasien yg siap akan masuk ke poli gigi //
            if($DataSuara[$i]["suara_id"]==$DataAntrian["antri_nomer"] && $DataAntrian["antri_suara"]=="A" && $DataAntrian["id_poli"]==$poliId && $DataAntrian["id_loket"]==$loketId) {
                $lokasi = $ROOT."antrian_reguler/suara_3"; // link folder suara poli umum .ogg //
                ...
                $sql = "update klinik.klinik_antrian_reguler set antri_suara = 'M' where antri_id = ".QuoteValue(DPE_NUMERIC,$DataAntrian["antri_id"])." 
                ...
                if($DataAntrian["antri_buffer_3"]!='0') { 
                  ...
                  if($DataAntrian["antri_buffer_1"]!='0') {
                    ...
                    $sql = "update klinik.klinik_antrian_reguler set antri_suara = 'A' , antri_nomer = ".QuoteValue(DPE_NUMERIC,$DataAntrian["antri_buffer_1"])." , id_poli = ".QuoteValue(DPE_CHAR,$DataAntrian["antri_buffer_poli_1"])." where antri_id = ".QuoteValue(DPE_NUMERIC,$DataAntrian["antri_id"])." and id_dep =".QuoteValue(DPE_CHAR,$depId);
                    $dtaccess->Execute($sql,DB_SCHEMA_KLINIK);
                    
                    $sql = "update klinik.klinik_antrian_reguler set antri_buffer_1 = ".QuoteValue(DPE_NUMERIC,$DataAntrian["antri_buffer_2"])." , antri_buffer_poli_1 = ".QuoteValue(DPE_CHAR,$DataAntrian["antri_buffer_poli_2"])." where antri_id = ".QuoteValue(DPE_NUMERIC,$DataAntrian["antri_id"])." and id_dep =".QuoteValue(DPE_CHAR,$depId);
                    $dtaccess->Execute($sql,DB_SCHEMA_KLINIK);
                    
                    $sql = "update klinik.klinik_antrian_reguler set antri_buffer_2 = ".QuoteValue(DPE_NUMERIC,$DataAntrian["antri_buffer_3"])." , antri_buffer_poli_2 = ".QuoteValue(DPE_CHAR,$DataAntrian["antri_buffer_poli_3"])." where antri_id = ".QuoteValue(DPE_NUMERIC,$DataAntrian["antri_id"])." and id_dep =".QuoteValue(DPE_CHAR,$depId);
                    $dtaccess->Execute($sql,DB_SCHEMA_KLINIK);
                    
                    $sql = "update klinik.klinik_antrian_reguler set antri_buffer_3 = ".QuoteValue(DPE_NUMERIC,'0')." , antri_buffer_poli_3 = ".QuoteValue(DPE_CHAR,'0')." where antri_id = ".QuoteValue(DPE_NUMERIC,$DataAntrian["antri_id"])." and id_dep =".QuoteValue(DPE_CHAR,$depId);
                    $dtaccess->Execute($sql,DB_SCHEMA_KLINIK);
 			    ...
 			    //a.reg_status like '".STATUS_PEMERIKSAAN.STATUS_PROSES."%' and
     ...
     if ($dataTable)
        ...
        if($dataTable[0]["id_poli"]==1) $poliNama=" ANTRIAN  NO";
        if($dataTable[0]["id_poli"]==2) $poliNama=" ANTRIAN  NO";
...
<?php echo $hasil;?>
<div width="100%">
<h3>LOKET 3</h3><?php //if($dataTable[0]["reg_antri_nomer"]) echo '<img class="pp" src="'.$fotoName.'" />'; ?>
<!-- <div class="nom">

&nbsp;<?php echo $poliNama;?>&nbsp;&nbsp;<?php if($dataTable[0]["reg_antri_nomer"]) echo $dataTable[0]["reg_antri_nomer"];?></div>-->
<font color='red'><div class="nam">&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $poliNama;?>&nbsp;&nbsp;<?php if($dataTable[0]["reg_antri_nomer"]) echo $dataTable[0]["reg_antri_nomer"];?></font></div>
