...
class CView
{
     ...
     function RenderPaging($in_totRecord,$in_recPerPage, $in_currPage = 1)
    {
        ...
        if ($in_currPage == 1) $strPaging .= "Awal   Sebelumnya ";
        else {
            $strPaging .= "<A HREF=\"".$this->_page."?currentPage="."1"."&recPerPage=".$in_recPerPage.$this->_newQString."\">Awal</A>\n";
            $strPaging .=  "<A HREF=\"".$this->_page."?currentPage=".($in_currPage - 1)."&recPerPage=".$in_recPerPage.$this->_newQString."\">Sebelumnya</A>\n";
        }
        $strPaging .= " | \n";
        ...
        if ($in_currPage == $pagingNum || $in_totRecord == 0) $strPaging .= "Berikutnya     Akhir\n"; 
            ...
            $strPaging .= "<A HREF=\"".$this->_page."?currentPage=".($in_currPage + 1)."&recPerPage=".$in_recPerPage.$this->_newQString."\">Berikutnya</A>\n";
            $strPaging .= "<A HREF=\"".$this->_page."?currentPage=".$pagingNum."&recPerPage=".$in_recPerPage.$this->_newQString."\">Akhir</A>\n";
        }
...
//		if ($in_month<10) $in_month = "0".$in_month;
     ...
     * @access public
     **/
	function RenderBody($in_css,$in_cssColor=null,$head=false,$isCal=false,$in_title=null,$in_event=null,$isGrid=false,$in_cssPrint=null)
	{
        global $APLICATION_ROOT,$ROOT,$iklanAtas,$myLogo,$warnaCSS,$logoKlinik;
        $logoKlinik='logo.png'; //sementara
        $strBody = '<html>'."\n";
        $strBody.= '<head>'."\n";
        $strBody.= '<meta http-equiv="Content-Type" content="text/html; charset=utf-8">'."\n";        
        $strBody.= '<meta name="description" content="Software Klinik Gigi berbasis Online Realtime, untuk Klinik anda,sangat mudah pengunaannya, simple dan praktis, sudah termasuk SMS Gateway, IPAD dan Antrian Pasien,fasilitas lengkap garansi lifetime dengan harga terjangkau." />'."\n"; 
        $strBody.= '<meta name="keywords" content="Aplikasi Klinik,Software Klinik,Sistem Informasi Klinik,Aplikasi klinik,Software klinik,Sistem Informasi klinik,Aplikasi Dokter Praktek,Software Dokter Praktek,Sistem Informasi Dokter Praktek, Aplikasi Klinik,Software dokter praktek,Sistem Informasi dokter praktek,Dokter,dokter,klinik,Klinik" />'."\n"; 
        $strBody.= '<link rel="canonical" href="http://sikita.net/" />'."\n"; 
        $strBody.= '<title>'.$in_title.'</title>'."\n";
        $strBody.= '<link href="'.$ROOT.'gambar/icon_pdam.png" rel="Shortcut Icon">'."\n";
        ...
        $strBody.= '<tr>';
        $strBody.= '<td width="10%" align="left" valign="top"><a href="http://sikita.net" target="_blank"><img src="'.$ROOT.'gambar/img_cfg/'.$logoKlinik.'" width="110" height="50"/></a></td>';
        $strBody.= '<td width="60%" align="left" valign="top"><a href="http://sikita.net" target="_blank"><div id="loading" style="display:none">Loading...</div><div class="reminder"><marquee onmouseover="this.stop();" onmouseout="this.start();">'.$iklanAtas.'</marquee></div></td>';
        $strBody.= '<td width="40%" valign="top" align="right"><a href="#" target="_blank"><font size="6">'.$in_title.'</font></a></td>';
        $strBody.= '</tr>';
        $strBody.= '</table>';
        $strBody.= '</div>';
        $strBody.= '<body';       
     ...
     **/
     ...
     function RenderBottom($in_bottom_css,$in_bottom_cssColor,$in_user=null,$keluar=false,$thedep=null,$logo_bottom=null,$marquee_bottom=null)
     {
...
class ExpTable
{
     ...
     function ExpTable($in_id,$in_width="100%",$in_align="center",$in_height=null,$in_class=null)
     {
          $this->_id = $in_id;
          $this->_width = $in_width;
          $this->_height = $in_height;
